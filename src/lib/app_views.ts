 ...
 * ⌘I toggles Hosts and ⌘, opens Settings (Cmd never reaches the PTY); on
 * non-mac Ctrl+Shift+H also toggles Hosts (Ctrl+Shift+I is the devtools
 * chord). Plain Ctrl chords stay with the terminal.
 */
export function appChord(
  ...
  if (!isMac && e.ctrlKey && e.shiftKey && !e.metaKey && k === 'h') return 'hosts';
  return null;
}
