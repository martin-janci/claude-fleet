...
/// (`rustls-native-certs`, via `fleet_core::net::tls`), not a bundled root set. That is deliberate: a
...
/// The type (and the connect/read code behind it) is `fleet_core::net::conn`'s
/// since work graph M3.0, shared with fleet-core's tracker client.
pub type HubStream = fleet_core::net::conn::Stream;
...
pub async fn connect(at: &Endpoint) -> Result<HubStream, String> {
    fleet_core::net::conn::connect(&at.at, CONNECT_TIMEOUT).await
}
...
{
    fleet_core::net::conn::speak(conn, host, port, request.as_bytes(), MAX_RESPONSE).await
}
