...
//! place — a one-shot call reads until the peer closes the connection, a
//! stream reads only up to the blank line that ends the head and then keeps
//! the socket open indefinitely — so the read loops themselves stay with
//! each caller (`remote::speak`, `events::open_stream`). This module owns
//! the part that is genuinely identical: parsing bytes once they are in
//! hand.
//!
//! TLS / connect / [`fleet_proto::net::Endpoint`] are already shared via
//! `remote::connect` and `remote::Endpoint`, which `events.rs` imports
