...
/// Where the catalog repo lives and its last-loaded HEAD (migration 018).
/// Singleton row (`id = 1`).
#[derive(Debug, Clone, serde::Serialize)]
...
mod tests {
    ...
    fn migrate_is_idempotent() {
        ...
        assert_eq!(store.schema_version().expect("version"), 18);
    }
    ...
    fn schema_version_is_seven_after_migration() {
        let s = Store::open_in_memory().expect("open");
        assert_eq!(s.schema_version().expect("version"), 18);
    }
