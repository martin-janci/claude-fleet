...
One JSON file holds 68 adjectives (colours, short evocative qualities) and 124
...
`generateName(existing, rng)` draws up to 24 random pairs not in `existing`,
...
Cmd/Ctrl+K and Cmd/Ctrl+P (captured before the terminal). One input; rows =
...
- No Sidebar changes (#46 rewrites it). Its project popover already caps at
  240px and scrolls; the tree scrolls; nothing there is a one-line fix.
...
- Sidebar: use `PickerList` for session rows so keyboard selection scrolls;
  switch its footer button to `requestNewSession` and drop its own dialog
  mount; show `friendly_name` from the generated pair by default.
