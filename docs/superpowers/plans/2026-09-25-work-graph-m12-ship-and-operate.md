...
- 2026-09-26: **M12.4 (trackers in fleet health) landed.** No migration, no new tool, no contract bump.
  - **Roll-up.** `fleet_health.trackers` (`service/health.rs`, `#[serde(default)]`, so an older hub's JSON still parses):
    - per tracker: `status`, stored `state`, `consecutive_failures`, `last_error` (redacted, defused, capped as M11.4's metric error), `last_success_at` (`trackers.last_sync_at`, stamped only by an ok pass) and `last_pass_at`;
    - fleet-wide: `failing` / `degraded` counts, and `detection_backlog`: live suggestions older than `backlog_days` (7) that a session row still shows. A weak suggestion behind a confirmed primary is not counted, since nobody is asked about it.
  - **Status rule** (`tracker_status`, one place):
    - `failing` when a person has to act (`auth_failed`, `captcha`) or after 3 failed passes in a row;
    - `degraded` for any other non-`ok` state (including `unconfigured`, "not tested yet", which raises no alarm), a failed last pass, or an error kept on an `ok` row (a 403 or 404 keeps the state);
    - `ok` otherwise.
  - **Cached, never live.** It reads the store and the sync's in-memory metrics. M11.4's `SyncMetrics` gained `consecutive_failures` (0 after an ok pass; in memory, so 0 after a restart until a pass runs).
  - **Isolation.** A per-host token sees the trackers `work { action: trackers }` shows it (its org's, narrowed by M3's host fence) and the backlog on its own host's sessions. The master and paired clients see the fleet. A read error fails closed to an empty roll-up. `fleet_health_shows_a_host_only_its_orgs_trackers_and_its_own_backlog` covers the six callers.
  - **Desktop.**
    - The footer shows "trackers: 1 failing · 4 suggestions waiting > 7 d", opens Settings → Work, and is re-read with the two-minute tracker refresh.
    - D22: one Attention item per tracker in `auth_failed` / `captcha`, "Reconnect Jira (acme)", in the sidebar's attention strip (`TrackerAttention.svelte`). It opens Settings → Work (`openSettingsAt`) and reads the live `trackers` store, so a `work:tracker` frame raises or clears it without polling. A transient failure (unreachable three times) shows as failing in health but raises no "Reconnect": reconnecting would not help.
    - Settings → Work's sync line adds "N failed in a row".
  - **Phone.** `fleet_health` was already in its tool list and the field is additive; nothing else changes.
  - **Surface.** The `fleet_health` description grew to 55,693 B of the 55,735 budget, so the constant is unchanged. The hub contract golden gains `Health.trackers`, `TrackersHealth` and `TrackerHealth` (additions only, same revision).
