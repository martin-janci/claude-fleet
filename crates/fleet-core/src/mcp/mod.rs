...
/// Build the axum app: `/mcp` (rmcp service), `/hook`, `/report` and
/// `/reports` behind [`authorize`], plus the unauthenticated `/healthz`
/// liveness and `/pair` exchange routes. Shared by `start` and the routing
/// test.
fn build_app(
